...
public class ModifyColor : MonoBehaviour {
    ...
    void BounceDown()
    {
        LeanTween.scale(gameObject, new Vector2(22F, 22F), .5F).setEase(LeanTweenType.easeInBack);
    }
    
    void BounceUp()
    {
        LeanTween.scale(gameObject, new Vector2(28F, 28F), .5F).setEase(LeanTweenType.easeOutBack);
    }
}
